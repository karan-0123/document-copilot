...
async def create_thread(client: AsyncClient, user_id: UUID | str, title: str | None = None) -> dict[str, Any]:
...
async def get_thread(client: AsyncClient, thread_id: UUID | str) -> dict[str, Any] | None:
    """Retrieve a thread by its ID."""
    response = await client.table("chat_threads").select("*").eq("id", str(thread_id)).execute()
    return response.data[0] if response.data else None

async def list_threads(client: AsyncClient, user_id: UUID | str) -> list[dict[str, Any]]:
...
async def update_thread(client: AsyncClient, thread_id: UUID | str, title: str) -> dict[str, Any]:
...
async def delete_thread(client: AsyncClient, thread_id: UUID | str) -> bool:
    ...
    response = await client.table("chat_threads").delete().eq("id", str(thread_id)).execute()
...
async def get_messages(client: AsyncClient, thread_id: UUID | str) -> list[dict[str, Any]]:
...
async def get_message_citations(client: AsyncClient, message_id: UUID | str) -> list[dict[str, Any]]:
