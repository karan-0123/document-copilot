...
Revision ID: bff5e3329676
Revises:
Create Date: 2026-06-24 19:50:59.273316
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_source_documents_doc_metadata_gin",
        "source_documents",
        ["doc_metadata"],
    ...
    op.create_index(
        "ix_document_chunks_chunk_metadata_gin",
        "document_chunks",
        ["chunk_metadata"],
        postgresql_using="gin",
    )
...
def downgrade() -> None:
    op.execute("DROP POLICY IF EXISTS document_chunks_read_authenticated ON document_chunks")
    op.execute("DROP POLICY IF EXISTS source_documents_read_authenticated ON source_documents")
    op.execute("DROP POLICY IF EXISTS message_citations_owner_all ON message_citations")
    op.execute("DROP POLICY IF EXISTS chat_messages_owner_all ON chat_messages")
    op.execute("DROP POLICY IF EXISTS chat_threads_owner_all ON chat_threads")
    ...
    op.drop_index("ix_document_chunks_chunk_metadata_gin", table_name="document_chunks")
    ...
    op.drop_index("ix_source_documents_doc_metadata_gin", table_name="source_documents")
